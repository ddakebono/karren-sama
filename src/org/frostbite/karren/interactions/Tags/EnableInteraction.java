...
public class EnableInteraction extends Tag {
    ...
    public String handleTemplate(String msg, Interaction interaction, MessageEvent event) {
        ...
        if(parameter!=null){
            ...
            for(Interaction enable : Karren.bot.getGuildManager().getInteractionProcessor(event.getChannel()).getInteractions().stream().filter((p)-> p.getIdentifier().equalsIgnoreCase(parameter)).collect(Collectors.toList())){
