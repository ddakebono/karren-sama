                ...
                if(jsonObject.getJSONArray("items").length()>0) {
                    if (Arrays.asList(interaction.getTags()).contains("feelinglucky")) {
                        ...
                        for (int i = 0; i < results.length(); i++) {
                            resultMessage += "\n" + (i+1) + " : " + results.getJSONObject(i).getJSONObject("snippet").getString("title");
                        }
                        resultArrays.put(event.getAuthor().getID(), results);
                        Karren.bot.getInteractionManager().getInteractionProcessor(event.getGuild()).getInteractions().add(new Interaction("selectOption", new String[]{"parameter", "d4jselect", "d4jplay"}, new String[]{"You selected \"%title\", added to queue!"}, new String[]{"That's not one of the options!"}, 1, event.getAuthor().getID(), interaction.getVoiceVolume()));
                        msg = msg.replace("%results", resultMessage);
                    }
                } else {
